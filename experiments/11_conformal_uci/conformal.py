...
log.info("=" * 56)
log.info("PHASE 6 — CONFORMAL PREDICTION — UCI")
log.info("=" * 56)
log.info(f"Device: {DEVICE}  |  Alpha: {ALPHA}  |  Target: {1-ALPHA:.0%} coverage")

scaler = pickle.load(open(ROOT / "models/uci/configs/scaler_uci.pkl", "rb"))
val_df = pd.read_csv(ROOT / "data/uci/splits/val.csv", index_col=0, parse_dates=True)

# Load actual MWh values from proxy CSV — already correct scale and alignment
proxy_df    = pd.read_csv(ROOT / "results/uci/tables/confidence_proxies_uci.csv", index_col=0, parse_dates=True)
test_actual = proxy_df["actual_load"].values         # MWh, 8592 rows
test_mean   = np.load(ROOT / "models/uci/lstm/all_predictions.npy").mean(axis=0)  # MWh, 8592
...
val_mean = get_val_ensemble_mean(val_df, scaler, ROOT / "models/uci/lstm", N_SEEDS, N_FEATURES, DEVICE)
...
conf_df.to_csv(ROOT / "results/uci/tables/conformal_uci.csv")
log.info("Saved -> results/uci/tables/conformal_uci.csv")

log.info("")
log.info("=" * 56)
log.info("UCI CONFORMAL COMPLETE")
log.info("=" * 56)
...
log.info(f"  Winkler Score: {ws:.4f}")
log.info("Next: python experiments/12_conformal_gefcom/conformal.py")
